...
def format_profile(user_id, name):
    ...
    return (
        ...
        f"👥 Приглашено друзей: {row[13] if len(row) > 13 else 0}"
    )
    ...
    if len(args) > 1:
        try:
            ...
            if ref_id != user.id:
                ...
                if ref_row and (len(ref_row) < 15 or ref_row[14] == 0):
                    conn = sqlite3.connect(DB_PATH)
                    c = conn.cursor()
                    c.execute("UPDATE users SET referred_by=? WHERE user_id=?", (ref_id, user.id))
                    c.execute("UPDATE users SET referral_count=referral_count+1, cash=cash+50 WHERE user_id=?", (ref_id,))
                    conn.commit()
                    conn.close()
                    try:
                        bot.send_message(ref_id, f"🎉 По вашей ссылке зарегистрировался {name}!\n\nНаграда: 💵 50")
                    except Exception:
                        pass
        except Exception:
            pass
...
def cmd_referral(message):
    ...
    ref_count = row[13] if row and len(row) > 13 else 0
    try:
        username = bot.get_me().username
        link = f"https://t.me/{username}?start=ref{user.id}"
    except Exception:
        link = f"Ошибка получения ссылки"
    bot.send_message(message.chat.id,
...
def cmd_endgame(message):
    ...
    if message.from_user.id != game['host'] and not is_admin(message.from_user.id):
        ...
        return
    end_game(chat_id, "🛑 Игра завершена!", forced=True)
...
def cb_shop_diamonds(call):
    ...
    markup.add(types.InlineKeyboardButton("🎭 Один правильный ответ — 💎 1", callback_data="buy_correct_answer_dia"))
    markup.add(types.InlineKeyboardButton("◀️ Назад", callback_data="shop_back"))
    bot.edit_message_text(
        f"💎 Магазин за алмазы\n\nБаланс: 💎 {diamonds}\n\n"
...
@bot.callback_query_handler(func=lambda call: call.data == "buy_correct_answer_dia")
def cb_buy_correct_answer_dia(call):
    row = get_user(call.from_user.id, call.from_user.first_name)
    if row[3] < 1:
        bot.answer_callback_query(call.id, "❌ Недостаточно алмазов!")
        return
...
def cb_adm_logs(call):
    ...
    conn.close()
    if not rows:
        bot.answer_callback_query(call.id, "Лог пуст!", show_alert=True)
        return
...
def cb_adm_s_resetall(call):
    if not is_super_admin(call.from_user.id): return
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("⚠️ Да, сбросить!", callback_data="adm_s_resetall_confirm"))
    markup.add(types.InlineKeyboardButton("❌ Нет", callback_data="adm_super"))
    try:
        bot.edit_message_text("⚠️ Вся статистика будет сброшена!\n\nПодтвердить?",
            call.message.chat.id, call.message.message_id, reply_markup=markup)
    except Exception:
        pass
...
def cb_adm_s_resetall_confirm(call):
    ...
    bot.answer_callback_query(call.id, "✅ Вся статистика сброшена!")
...
def cmd_addcash(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, f"✅ Игроку {user_id} добавлено 💵 {amount}!")
    except Exception as e:
...
def cmd_ban(message):
    ...
    bot.send_message(message.chat.id, f"🚫 Игрок {user_id} заблокирован!")
...
def cmd_unban(message):
    ...
    bot.send_message(message.chat.id, f"✅ Игрок {user_id} разблокирован!")
...
def cmd_mute(message):
    ...
    bot.send_message(message.chat.id, f"🔇 Игрок {user_id} замьючен!")
...
def cmd_setdonor(message):
    ...
    bot.send_message(message.chat.id, f"⭐ Игроку {user_id} выдан статус донатора!")
...
def cmd_resetuser(message):
    ...
    bot.send_message(message.chat.id, f"✅ Профиль игрока {user_id} сброшен!")
...
def cmd_addadmin(message):
    ...
    bot.send_message(message.chat.id, f"✅ Игрок {user_id} добавлен в админы!")
...
def cmd_removeadmin(message):
    ...
    bot.send_message(message.chat.id, f"✅ Игрок {user_id} убран из админов!")
...
def cmd_game(message):
    ...
    # Тек группа админдері немесе бот админдері
    try:
...
def start_the_game(chat_id):
    ...
    if not enabled_locs:
        bot.send_message(chat_id, "❌ Все локации отключены!")
        return
    ...
    game.update({
        ...
        'last_activity': time.time(), 'eliminated': [], 'rifle_used': False, 'rifle_locked': {}
    })
...
def send_next_player_question(chat_id):
    ...
    has_correct = row[6] > 0 if row else False
    ...
    # Барлық ойыншыда 3 зат көрінеді
...
def cb_use_rifle(call):
    ...
    if call.from_user.id in game.get('rifle_locked', {}):
        bot.answer_callback_query(call.id, "❌ Вы уже ответили! Винтовку нельзя использовать.", show_alert=True)
        return
...
def cb_rifle_shoot(call):
    ...
    row = get_user(call.from_user.id, call.from_user.first_name)
    if not row or row[7] < 1:
        bot.answer_callback_query(call.id, "❌ Нет винтовки!")
        return
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("use_correct_"))
def cb_use_correct(call):
    parts = call.data.split("_")
    chat_id = int(parts[2])
    player_id = int(parts[3])
    if call.from_user.id != player_id:
        bot.answer_callback_query(call.id, "❌ Это не твой предмет!")
        return
    game = get_game(chat_id)
    if not game:
        bot.answer_callback_query(call.id, "❌ Игра не найдена!")
    ...
    if not row or row[6] < 1:
        bot.answer_callback_query(call.id, "❌ У вас нет этого предмета. Купите в /shop", show_alert=True)
        return
    ...
    bot.answer_callback_query(call.id)
    bot.send_message(call.from_user.id, f"🎭 Правильный ответ на вопрос:\n\n✅ {correct}")
